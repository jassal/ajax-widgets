...
*(note)Note:* this only occurs when adding a new row to a matrix.  The user is responsible for any edits the made to the data beyond this point.

Also, the *Att Name* property for a column should exist and be unique for default values to work.

h2. TimePicker

The default value for a time picker should be in the format *h:m:s* or *h:m*.  Milliseconds cannot be used.  The hour, minute, and optional seconds can be one or two digits.
...
The default value for a checkbox can be set with either @true/false@ or @1/0@.  Actually, any value that is not @true@ or @1@ will be considered false.
