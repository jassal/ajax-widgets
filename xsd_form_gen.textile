...
pre(xml-format). <?xml version='1.0' encoding='UTF-8'?>
<xs:schema 
...
The above XSD will generate the following form.  The @<annotation>@ elements are optional and are used to set the form label.  As you can see the rest of the schema is typical.  You can download this XSD "here (Name.xsd)":files/Name.xsd.
...
You can use enumerations to populate values for lists, radio buttons, and checkboxes.  The following will generate a select field with the enumeration values as the select list.  
...
Here are the default input fields created from the given XSD data types. 
...
Again, these values can be overridden with the use of the *widgetType* annotation.  For instance, the following example uses a textarea instead of the default textinput.

pre(xml-format). <xs:element name="instructions" type="xs:string">
  <xs:annotation us:label="Instructions" us:widgetType="textarea"/>
</xs:element>  
...
*(note)Note:* If you add, remove, or rename form fields, then you will need to make changes to the mapping file manually.  Currently Intalio|Ajax doesn't modify the mapping file automatically when the form is generated from a schema.
